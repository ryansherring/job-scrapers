...
def linkedin():
    ...
    for job in job_container:
        ...
        links = f"https://www.linkedin.com/jobs/search/?currentJobId={job_ids}&redirect=false"
        links.append(job_ids)
    ...
    exclude_job_data = pd.DataFrame({'Title': post_title,
    'links': links,
    'Date': post_date,
    ...
    include_job_data = pd.DataFrame({'Title': post_title,
    'links': links,
    'Date': post_date,
    'Company Name': company_name,
    'Location': job_location,
    ...
    if(len(linkedin_data.index) > 0):
        print(linkedin_data.info())
        # linkedin_data.to_csv('./jobs.csv', index=0)
